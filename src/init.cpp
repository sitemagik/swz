...
void RestartWallet(const char *parm, bool fOldParms)
{
    ...
    if (fNewVersion && !fBootstrapTurbo && !fRescan && !fEncrypt) // fNewVersion could be true while trying to do other restarts
    {
        ...
        command = QString(GetDataDir().string().c_str()) + QString("/") + QString(GetArg("-vFileName","sweet-setup.exe").c_str());
#else
#ifdef MAC_OSX
        ...
        newArgv.append(QString(GetDataDir().c_str()) + QString("/") + QString(GetArg("-vFileName","sweet-setup.pkg").c_str()));
#else
        // If Linux, just restart (already extracted sweet-qt from the zip in downloader.cpp).
        parm = NULL;
        fOldParms = false;
        newArgv.clear();
        // Installer created by makeself.sh
        command = QString(GetDataDir().c_str()) + QString("/") + QString(GetArg("-vFileName","sweet-setup.run").c_str());
        newArgv.append(QString("--target"));
        newArgv.append(QString(GetProgramDir().c_str()));
        newArgv.append(QString("--nox11"));
        // Make executable
        boost::filesystem::path installer(GetDataDir() / GetArg("-vFileName","sweet-setup.run"));
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("sweet Version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  sweetd [options]                     " + "\n" +
                  "  sweetd [options] <command> [params]  " + _("Send command to -server or sweetd") + "\n" +
                  "  sweetd [options] help                " + _("List commands") + "\n" +
                  "  sweetd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "sweet:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("sweet"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("sweet"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file path (default: <cwd>/sweet.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: sweetd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  sweet is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "sweet server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("sweet"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of sweet") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart sweet to complete") << "\n";
