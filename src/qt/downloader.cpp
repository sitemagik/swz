...
void Downloader::startRequest(QUrl url)
{
    ...
    if (processBlockchain)
    {
        statusText.append(tr("\n\nThe sweet wallet will restart after extracting the bootstrap%1").arg((fEncrypt ? " and encrypting the wallet." : ".")));
    }
    else if (processUpdate)
    {
        statusText.append(tr("\n\nThe sweet wallet will restart after downloading the update."));
    }
