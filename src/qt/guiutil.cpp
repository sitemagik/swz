...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(BitcoinUnits::decimals(BitcoinUnits::swzo));
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::swzo, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert sweet:// to sweet:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sweet://"))
    {
        uri.replace(0, 12, "sweet:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "sweet.lnk";
}
...
{
    return GetAutostartDir() / "sweet.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("sweet-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sweet-qt [" + tr("command-line options") + "]                     " + "\n";
